...
public class IndirectTranslationsExperimentV2 {
	...
	public ArrayList<TranslatablePair> createTranslatablePairsWithScoreFile (String sparqlEndpoint, String sourceLanguage, String pivotLanguage, String targetLanguage, String outputFile){
		...
		try {			
			...
			for ( ; results.hasNext() ; )    {		
				...
				//just for testing - no need to generate the whole dictionary, 
				//remove when testing is done
				if (translatablePairs.size()>=4) {break;}  
			}
			writer.close();
